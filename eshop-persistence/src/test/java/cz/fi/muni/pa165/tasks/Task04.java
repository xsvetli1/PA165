...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductsWithCategories(){
		List<Product> found = em.createQuery("select p from Product p join fetch p.categories c",Product.class).getResultList();
	...
	public void groupByAndOrderBy(){
		List<Object[]> found= em.createQuery("select color, count(color) from Product p group by color order by p.color", Object[].class).getResultList();
	...
	public void groupByAndOrderBySelectNew() {
		List<ColorCount> colorCounts = em.createQuery("select new cz.fi.muni.pa165.tasks.ColorCount(color, count(color)) from Product p group by color order by p.color",ColorCount.class).getResultList();
	...
	public void findProductsAddedYesterday() {
		...
		List<Product> products= em.createQuery("select p from Product p where p.addedDate = :date",Product.class).setParameter("date", cal.getTime()).getResultList();
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em.createQuery(
				"select new cz.fi.muni.pa165.tasks.ProductAndCategory(p, c)" +
						"from Product p left join Category c on c.name = 'Kitchen'" +
						"and c in (select cat from p.categories cat)" +
						"order by p.name",
				ProductAndCategory.class).getResultList();
	...
	public void criteriaFindProductsWithNonEmptyCategory() {
		...
		Root<Product> product = query.from(Product.class);
		query.select(product);
		query.where(cb.isNotEmpty(product.get("categories")));
		List<Product> found = em.createQuery(query).getResultList();
		Assert.assertEquals(found.size(), 3);
	}
}
